...
static bool g_setBrightness = false;
static bool g_setTemperature = false;
...
class GammaTest : public QObject
{
    Q_OBJECT
public:
    explicit GammaTest(QObject *parent = nullptr);
    virtual ~GammaTest();
    void init();
    void setColorCurves(OutputDevice* outputDevice, float brightness);
    void setColorCurves(OutputDevice* outputDevice, int temperature);
private:
    ...
    OutputDevice *m_outputDevice0 = nullptr;
    OutputDevice *m_outputDevice1 = nullptr;
};

GammaTest::GammaTest(QObject *parent)
    : QObject(parent)
...
void GammaTest::setColorCurves(OutputDevice* outputDevice, float brightness)
{
    ...
    qDebug() << "========== setColorCurves, gamma size is " << rampsize;
    m_outputConfiguration->setColorCurves(outputDevice, vecRed, vecGreen, vecBlue);
    m_outputConfiguration->apply();
}

void GammaTest::setColorCurves(OutputDevice* outputDevice, int temperature)
{
    OutputDevice::ColorCurves colorCurves = outputDevice->colorCurves();
...
void GammaTest::setupRegistry(Registry *registry)
{
    ...
    connect(registry, &Registry::outputDeviceAnnounced, this,
        [this, registry] (quint32 name, quint32 version) {
            auto outputDevice = registry->createOutputDevice(name, version, this);
            if (outputDevice) {
                if (!m_outputDevice0) {
                    m_outputDevice0 = outputDevice;
                    connect(m_outputDevice0, &OutputDevice::changed, this, [=]() {
                        //qDebug() << m_outputDevice0->model() << " " << m_outputDevice0->uuid();    
                    });
                    qDebug() << QDateTime::currentDateTime().toString(QLatin1String("hh:mm:ss.zzz ")) << "createOutputDevice";
                    m_outputDevice0->setEventQueue(m_eventQueue);

                    connect(m_outputDevice0, &OutputDevice::colorCurvesChanged, this, [=]() {
                        qDebug() << m_outputDevice0->model() << " " << m_outputDevice0->uuid();
                        if (g_setBrightness) {
                            this->setColorCurves(m_outputDevice0, g_brightness);
                        } else if (g_setTemperature) {
                            this->setColorCurves(m_outputDevice0, g_temperature);
                        }
                        
                    });
                }
                if (!m_outputDevice1 && (outputDevice != m_outputDevice0)) {
                    m_outputDevice1 = outputDevice;
                    connect(m_outputDevice1, &OutputDevice::changed, this, [=](){
                        //qDebug() << m_outputDevice1->model() << " " << m_outputDevice1->uuid();    
                    });
                    qDebug() << QDateTime::currentDateTime().toString(QLatin1String("hh:mm:ss.zzz ")) << "createOutputDevice";
                    m_outputDevice1->setEventQueue(m_eventQueue);

                    connect(m_outputDevice1, &OutputDevice::colorCurvesChanged, this, [=]() {
                        qDebug() << m_outputDevice1->model() << " " << m_outputDevice1->uuid();
                        if (g_setBrightness) {
                            this->setColorCurves(m_outputDevice1, g_brightness);
                        } else if (g_setTemperature) {
                            this->setColorCurves(m_outputDevice1, g_temperature);
                        }
                    });
                }
            }
...
{
    if (argc != 3) {
        qDebug() << "Invalid argument count";
        return -1;
    }
    for (int i = 1; i < argc; i++) {
        if (!strcmp("-brightness", argv[i]) || !strcmp("--brightness", argv[i])) {
            g_setBrightness = true;
            sscanf(argv[i+1], "%f", &g_brightness);
            break;
        } else if (!strcmp("-temperature", argv[i]) || !strcmp("--temperature", argv[i])) {
            g_setTemperature = true;
            sscanf(argv[i+1], "%d", &g_temperature);
            break;
        }
    }
